...
import pygame, math, io, time, threading, uuid
...
def render(room):
    ...
    if anim:
        ...
        if t >= 1:
            ...
            surface.blit(pf.render(sym, True, (0,0,0)), (x-25,y-25))
...
def frame(rid):
    ...
    if not room:
        return '',404
    return Response(frame_bytes(room), mimetype='image/png')
...
# ✅ hover route
@app.route('/hover/<rid>', methods=['POST'])
def hover(rid):
    room = rooms.get(rid)
    if not room:
    ...
    tile = g.from_pixel(d["x"], d["y"], WIDTH, HEIGHT, zoom=ZOOM)
    room["hover"] = tile
